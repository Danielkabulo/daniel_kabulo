  ...
  return (
    <div className="w-full h-full flex">
      <aside className="sidebar">
        <div className="mb-6">
          <h1 className="text-xl font-black tracking-tighter uppercase" style={{color:'var(--text-main)'}}>Kamoa<br/><span style={{color:'var(--accent)'}}>Supervision</span></h1>
        </div>

        <div style={{background:'var(--accent)'}} className="p-6 rounded-[28px] text-center text-white shadow-xl mb-6">
        ...
        <div className="px-2 mb-4 space-y-4">
          <div className="startup-group p-3">
            <div className="flex items-center gap-2 mb-2 px-1">
              <i className="fas fa-mountain text-[9px]" style={{color:'var(--accent)'}}></i>
          ...
          <div className="startup-group p-3">
            <div className="flex items-center gap-2 mb-2 px-1">
              <i className="fas fa-arrow-down text-[9px]" style={{color:'var(--accent)'}}></i>
        ...
        <div className="px-2 mb-4">
          <button onClick={() => setShowHistory(s => !s)} className="w-full flex items-center justify-between p-4 rounded-2xl transition-all border group" style={{background:'var(--card)', borderColor:'var(--border)'}}>
            <div className="flex items-center gap-3">
              <i className="fas fa-list-ul" style={{color:'var(--accent)'}}></i>
              <span className="text-[11px] font-black uppercase tracking-widest" style={{color:'var(--text-main)', opacity:0.6}}>Activités</span>
            </div>
            <i className={`fas fa-chevron-right text-[10px] transition-transform ${showHistory ? 'transform rotate-90' : ''}`} style={{color:'var(--text-muted)'}}></i>
          </button>
        </div>

        <div className="mt-auto pt-4 border-t flex gap-2" style={{borderColor:'var(--border)'}}>
          <button id="t-ios" onClick={() => setTheme('ios')} className="theme-btn active"><i className="fab fa-apple"></i></button>
          <button id="t-scada" onClick={() => setTheme('scada')} className="theme-btn"><i className="fas fa-terminal"></i></button>
          <button id="t-classic" onClick={() => setTheme('classic')} className="theme-btn"><i className="fas fa-desktop"></i></button>
        </div>
      </aside>

      <main className="main-stage">
        {/* History panel */}
        <div style={{position:'absolute', top:20, left:20, width:340, zIndex:100, transition:'all 0.3s'}} className={`${showHistory ? '' : 'hidden'}`}>
          <div className="glass-panel p-4">
            <div className="p-2 border-b flex justify-between items-center" style={{borderColor:'var(--border)'}}>
        ...
        <div className="max-w-5xl mx-auto space-y-6">
          <section className="glass-panel p-6">
            <h2 className="text-[10px] font-black uppercase tracking-[0.2em] mb-4" style={{color:'var(--accent)'}}>Réseau Belts Kamoa 1</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="glass-panel p-6 space-y-4">
              <div className="flex items-center gap-4">
                <div style={{background:'var(--accent)'}} className="w-12 h-12 rounded-2xl flex items-center justify-center text-white shadow-lg">
              ...
              <div className="space-y-3">
                <input value={description} onChange={e => setDescription(e.target.value)} placeholder="Describe fault in English..." className="ios-input" />
                <div className="grid grid-cols-2 gap-4">
                  <button onClick={() => processReport('STOPPED','⛔')} className="status-btn bg-red-500 shadow-lg text-white py-4 rounded-xl"> <i className="fas fa-power-off text-2xl mb-1"></i> <div className="text-[10px]">REPORT STOP</div></button>
                  <button onClick={() => processReport('RUNNING','✅')} className="status-btn bg-green-500 shadow-lg text-white py-4 rounded-xl"> <i className="fas fa-play text-2xl mb-1"></i> <div className="text-[10px]">REPORT START</div></button>
                </div>
              </div>
            </div>
            ...
            <div className={`glass-panel p-6 flex flex-col justify-between ${showPreview ? '' : 'hidden'}`}>
              ...
              <button onClick={copyToClipboard} style={{background:'var(--accent)'}} className="mt-4 w-full text-white py-3 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl">
          ...
          <section className="glass-panel p-6">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-[10px] font-black uppercase tracking-[0.2em]" style={{color:'var(--accent)'}}>Bibliothèque Pannes</h2>
            </div>
            ...
            <div className="mb-6 flex flex-wrap gap-2 items-end">
              ...
              <div className="w-32">
                ...
                <select ref={newFaultCatRef} className="ios-input-small">
              ...
              <button onClick={addNewFault} style={{background:'var(--accent)'}} className="p-2 h-[34px] rounded-xl text-white text-[10px] font-black px-4 shadow-lg">
